...
def GPSIMU(data):
    ...
    if (gps_count == 2):
        ...
        vehicle.ns = "/vehicle"
        vehicle.type = vehicle.MESH_RESOURCE
        vehicle.action = vehicle.ADD
        vehicle.lifetime = rospy.Duration(0)
        vehicle.id = 1
        #converted x, y position from gps lat, lng
        vehicle.pose.position.x = -0.6 + msg.pose.pose.position.x
        vehicle.pose.position.y = -0.3 + msg.pose.pose.position.y
        vehicle.pose.position.z = 0
        vehicle.pose.orientation.x = 0
        vehicle.pose.orientation.y = 0
        #heading point 
        vehicle.pose.orientation.z = msg.pose.pose.position.z #this is yaw data # 4.5#
        ...
        vehicle.scale.z = 1.0
        vehicle.color.a = 0.0
        vehicle.color.r = 0.0 #0.31
        vehicle.color.g = 0.0 #0.98
        vehicle.color.b = 0.0 #0.48
        #publish now vehicle's position
        vehicle.mesh_resource = "package://pr2_description/meshes/formul.dae"
        vehicle.mesh_use_embedded_materials = True
        pub.publish(vehicle)
